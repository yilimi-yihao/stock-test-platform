...
class TushareSource(BaseTushareSource):
    """Tushare 数据源"""
    ...
    def get_stock_info(self, code: str, capabilities: Optional[dict[str, bool]] = None) -> Dict[str, Any]:
        """获取股票详细信息"""
        info = {
            'code': code,
            'name': code,
        ...
        try:
            ...
            if capabilities is None or capabilities.get('stock_basic', True):
            ...
            if capabilities is None or capabilities.get('daily_basic', True):
            ...
            if capabilities is None or capabilities.get('adj_factor', True):
    ...
    def detect_capabilities(self, sample_code: str = '002594') -> list[dict[str, Any]]:
        """检测当前账号可用接口"""
        ts_code = self._to_ts_code(sample_code)
        checks = [
            ('stock_basic', '股票基础信息', lambda: self.pro.stock_basic(exchange='', list_status='L', fields='ts_code,symbol,name,area,industry,list_date')),
            ('daily', '日线行情', lambda: self.pro.daily(ts_code=ts_code, start_date='20240101', end_date='20241231')),
            ('daily_basic', '每日指标', lambda: self.pro.daily_basic(ts_code=ts_code, start_date='20240101', end_date='20241231', fields='ts_code,trade_date,pe,pb,total_mv,circ_mv,turnover_rate,volume_ratio')),
            ('adj_factor', '复权因子', lambda: self.pro.adj_factor(ts_code=ts_code, start_date='20240101', end_date='20241231')),
            ('fina_indicator', '财务指标', lambda: self.pro.fina_indicator(ts_code=ts_code, period='20240930', fields='ts_code,end_date,eps,roe,roa,netprofit_margin,grossprofit_margin')),
            ('income', '利润表', lambda: self.pro.income(ts_code=ts_code, period='20240930', fields='ts_code,end_date,total_revenue,n_income')),
            ('balancesheet', '资产负债表', lambda: self.pro.balancesheet(ts_code=ts_code, period='20240930', fields='ts_code,end_date,total_assets,total_liab,total_hldr_eqy_exc_min_int')),
            ('cashflow', '现金流量表', lambda: self.pro.cashflow(ts_code=ts_code, period='20240930', fields='ts_code,end_date,n_cashflow_act,n_cashflow_inv_act,n_cash_flows_fnc_act')),
        ]
